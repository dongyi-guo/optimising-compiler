...
class CFG {
    ...
    /**
     * Output a dotty file for a list of nodes
     * @param nodes: List of nodes for all basic blocks
     */
    public static void outputGraph(ArrayList<Node> nodes) {

        try {

            File writename = new File("input.dot");
            writename.createNewFile();

            BufferedWriter out = new BufferedWriter(new FileWriter(writename));
            out.write("graph" + "\"" + "main" + "\"" + "{");
            out.write("start -- " + nodes.get(0).name + ";");
    ...
    public static void main(String[] args) {
        ...
        ArrayList<Node> nodes = build(filename);
        outputGraph(nodes);
    }
}
